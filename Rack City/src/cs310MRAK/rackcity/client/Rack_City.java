...
//				LatLng pos = LatLng.newInstance(LatVal, LonVal);
	...
	private void addRacker(String a, LatLng p, int rn, int s, int cs, int r, int type)
	{
		...
		{
			String newp = p.toString();
			BikeRack tmp = new BikeRack(a, newp, r, rn, cs, s);
			listofracks.add(tmp);
		}
...
//		Query q = new Query("Rack");
//		PreparedQuery pq = datastore.prepare(q);
//		for (Entity res: pq.asIterable())
//		{
//			String addr = (String) res.getProperty("addr");
//			String LL = (String) res.getProperty("name");
//			double rate = (double) res.getProperty("rat");
//			int rackN = (int) res.getProperty("numR");
//			double cs = (double) res.getProperty("css");
//			int stolenN = (int) res.getProperty("stel");				
//			BikeRack br = new BikeRack(addr, LL, rate, rackN, (int)cs, stolenN);
//			
//			listofracks.add(br);
//		}
//	}
	...
	private void assignrackOutput (ArrayList<String[]> result)
	{
		...
		for(int i = 0; i <= result.size(); i++)
		{
			...
			BikeRack br = new BikeRack (addr, LL, Double.parseDouble(rate), Integer.parseInt(rackN), Integer.parseInt(cs), Integer.parseInt(stolenN));				
			listofracks.add(br);
		}
